        ...
        stage('Deploy to GKE') {
            steps {
                script {
                    ...
                    step([$class: 'KubernetesEngineBuilder', projectId: env.PROJECT_ID, clusterName: env.CLUSTER_NAME, location: env.LOCATION, manifestPattern: 'kubernetes/deployment.yaml', credentialsId: env.K8S_CREDENTIALS_ID, verifyDeployments: true])
                }
            }
        }
    }
